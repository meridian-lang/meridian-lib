...
## Metrics
- Sections: 23/37 inert (62% inert ratio)
- Operational inert: 0
- Unclassified inert: 0
- Inert categories: reference-documentation=21, template=1, tools-metadata=1
- Judgment: 8 blocks, 34 lines

### Inert section details
- L26 `Prerequisites`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L47 `Phase 0: ClawVisor Setup (Required for API Access)`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L89 `If the user declines ClawVisor`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L162 `Via ClawVisor`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L179 `Processing rules`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L192 `Quality gate`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L204 `Fetch events`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L219 `Brain structure`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L230 `Entity enrichment`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L243 `Strategy: Smart sampling, not bulk import`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L252 `Processing`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L260 `Filtering rules`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L279 `Supported formats`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L285 `Processing`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L300 `Quality rule`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L309 `Data sources`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L316 `Brain structure`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L326 `Processing`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L334 `Phase 7: File Archives`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L369 `Post-Bootstrap Checklist`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L432 `Resume Protocol`: reference-documentation — Reference documentation, rationale, examples, or changelog.
- L441 `Output Format`: template — Template/output shape is metadata unless explicit output assertions are authored.
- L461 `Tools Used`: tools-metadata — Tools sections are metadata-mining, not workflow execution.
...
+## The Priority Stack (( role: procedure ))
+
+use judgment to follow the The Priority Stack guidance:
+  Data sources ranked by **information density × ease of import**:
+  
+  | Priority | Source | Why | Time | Pages Created |
+  |----------|--------|-----|------|---------------|
...
-### Discovery
-
+## Phase 1: Existing Markdown / Obsidian Import (( role: procedure ))
+
...
-
+  
+  item: Run link extraction: `gbrain extract links --source db`
+  item: Run timeline extraction: `gbrain extract timeline --source db`
+  item: Start embeddings: `gbrain embed --stale` (runs in background)
+  
 > **Track progress:**
 > ```bash
...
-### Via ClawVisor
-
+## Phase 2: Google Contacts → People Pages (( role: procedure ))
+
...
+  item: Name, email(s), phone(s), company, title
+  item: Source attribution: `[Source: Google Contacts, YYYY-MM-DD]`
+  item: Any notes from the contact as initial context
...
-### Fetch events
-
+## Phase 3: Google Calendar (Last 90 Days) (( role: procedure ))
+
...
-
+## Phase 4: Gmail (Recent Threads) (( role: procedure ))
+
...
-
+  item: noreply@, no-reply@, notifications@, support@, mailer-daemon@
+  item: Unsubscribe-heavy senders (marketing)
+  item: GitHub/Jira/Linear notification emails
+  item: Calendar invites (already captured in Phase 3)
+  
 **Always import:**
...
-### Processing
-
+  item: Direct emails from people in the brain
+  item: Starred/flagged emails
+  item: Emails the user sent (their words are highest-value signal)
+
+## Phase 5: Conversation Exports (ChatGPT / Claude / Perplexity) (( role: procedure ))
+
...
+  
+  item: **ChatGPT:** Settings → Data Controls → Export → `conversations.json`
+  item: **Claude:** Download from claude.ai conversation history
+  item: **Perplexity:** Export from settings
+  
+### Processing (( inert ))
+  
...
+  item: 1 = Pure utility (how-tos, quick lookups) → skip or minimal page
+  item: 2 = Minor context → 1-paragraph note
+  item: 3 = Notable (reveals interests, building something) → full page
+  item: 4 = Important (deep personal processing, strategic thinking) → rich page
+  item: 5 = Defining (identity work, breakthrough insights) → full treatment
...
-### Data sources
-
+## Phase 6: X/Twitter Archive (( role: procedure ))
+
...
+  item: Contains all tweets, likes, DMs, bookmarks
...
+### Processing (( inert ))
+  
+  item: **Original tweets** → capture with full context, extract entities
+  item: **Quote tweets** → capture the user's commentary + the source tweet
+  item: **Threads** → reconstruct as a single narrative
+  item: **Bookmarks** → high-signal curation, import with tags
+  item: **Likes** — low signal, skip unless the user wants them
+
...
+## Phase 8: Meeting Transcripts (( role: procedure ))
+
