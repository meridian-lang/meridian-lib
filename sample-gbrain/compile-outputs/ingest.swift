...
public struct IngestInput: MeridianWorkflow {
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let __meridianProseResults_L34 = try await runtime.executeAutonomousLoop(
            ...
            scopedTools: ["add_link", "add_tag", "add_timeline_entry", "get_backlinks", "get_page", "get_tags", "link.add", "page.get", "page.search", "publish", "put_page", "put_raw_data", "shell.run", "timeline.add"],
            maxSteps: 32,
        ...
        // L69
        let __meridianProseResults_L69 = try await runtime.executeProsePlan(
        ...
        for (__key, __value) in __meridianProseResults_L69 {
            state.bind(__key, __value)
        }
        if __meridianShouldRun("progress:0.3:L74:C0") {
            // L74
            ...
            try await runtime.checkpoint(label: "progress:0.3:L74:C0", state: state.snapshot())
        }
        // L205
        let __meridianProseResults_L205 = try await runtime.executeProsePlan(
            prose: "follow the Raw Source Preservation guidance\nEvery ingested item must have its raw source preserved for provenance\n**Use `gbrain files upload-raw` for automatic size routing:**\ncodeblock:bash:Z2JyYWluIGZpbGVzIHVwbG9hZC1yYXcgPGZpbGU+IC0tcGFnZSA8cGFnZS1zbHVnPiAtLXR5cGUgPHR5cGU+\nitem: **< 100 MB text/PDF**: stays in git (brain repo `.raw/` sidecar directories)\nitem: **>= 100 MB OR media** (video, audio, images): uploaded to cloud storage\nvia TUS resumable upload, `.redirect.yaml` pointer left in the brain repo\nThe `.redirect.yaml` pointer format:\ncodeblock:yaml:\n**Accessing stored files:**\nitem: `gbrain files signed-url <storage-path>` -- generate 1-hour signed URL for viewing/sharing\nitem: `gbrain files restore <dir>` -- download back to local from cloud storage\nUse `put_raw_data` in gbrain to store raw API responses and metadata (JSON, not binary)\nchecklist:ai-autonomy:",
            snapshot: state.snapshot(),
            scopedTools: ["add_link", "add_tag", "add_timeline_entry", "get_backlinks", "get_page", "get_tags", "link.add", "page.get", "page.search", "publish", "put_page", "put_raw_data", "shell.run", "timeline.add"]
        )
        for (__key, __value) in __meridianProseResults_L205 {
            state.bind(__key, __value)
        }
