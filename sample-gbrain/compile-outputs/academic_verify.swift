...
public struct AcademicVerifyInput: MeridianWorkflow {
    ...
    public static let skillMetadata: [String: String] = [
        ...
        "triggers": "\"verify this academic claim\"\n\"check this study\"\n\"academic verify\"\n\"validate citation\"\n\"is this study real\"\n\"Retraction Watch\"",
        "mutating": "true",
        "writes_pages": "true",
        "writes_to": "concepts/",
    ]
...
public struct WhenVerifythisacademicclaimFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenVerifythisacademicclaimFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("verifyThisAcademicClaim", matching: nil))
        ...
        if __meridianShouldRun("progress:0.1:L1:C0") {
            ...
            try await runtime.emit(
                event: "trigger.verifyThisAcademicClaim.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("verify this academic claim"),
                ]
            )
...
public struct WhenCheckthisstudyFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenCheckthisstudyFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("checkThisStudy", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.checkThisStudy.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("check this study"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenAcademicverifyFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenAcademicverifyFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("academicVerify", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.academicVerify.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("academic verify"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenValidatecitationFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenValidatecitationFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("validateCitation", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.validateCitation.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("validate citation"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenIsthisstudyrealFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenIsthisstudyrealFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("isThisStudyReal", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.isThisStudyReal.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("is this study real"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenRetractionwatchFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenRetractionwatchFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("retractionWatch", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.retractionWatch.fired",
                payload: [
                    "kind": .string("ambient"),
                    "spec": .string("Retraction Watch"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
