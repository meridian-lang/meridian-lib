    ...
    public func run() async throws -> WorkflowResult {
        ...
        if __meridianShouldRun("progress:0.0:L51:C0") {
            ...
            _ = try await runtime.invoke(
                tool: "shell.run",
                args: [
                    "command": .string("gbrain config get self_upgrade.mode   # auto | notify | off  (default: notify)"),
                ]
            )
...
public struct WhenGbrainupdateavailableFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenGbrainupdateavailableFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("gbrainUpdateAvailable", matching: nil))
        ...
        if __meridianShouldRun("progress:0.1:L1:C0") {
            ...
            try await runtime.emit(
                event: "trigger.gbrainUpdateAvailable.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("gbrain update available"),
                ]
            )
...
public struct WhenUpgradeavailableFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenUpgradeavailableFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("upgradeAvailable", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.upgradeAvailable.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("UPGRADE_AVAILABLE"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenUpgradegbrainFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenUpgradegbrainFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("upgradeGbrain", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.upgradeGbrain.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("upgrade gbrain"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenUpdategbrainFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenUpdategbrainFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("updateGbrain", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.updateGbrain.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("update gbrain"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenGbrainisoutdateFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenGbrainisoutdateFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("gbrainIsOutDate", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.gbrainIsOutDate.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("gbrain is out of date"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenGbrainselfupgradeFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenGbrainselfupgradeFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("gbrainSelfUpgrade", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.gbrainSelfUpgrade.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("gbrain self-upgrade"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenIsgbrainupdateFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenIsgbrainupdateFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("isGbrainUpDate", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.isGbrainUpDate.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("is gbrain up to date"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenKeepgbraincurrentFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenKeepgbraincurrentFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("keepGbrainCurrent", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.keepGbrainCurrent.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("keep gbrain current"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
