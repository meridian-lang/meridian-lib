...
public struct RepoArchitectureInput: MeridianWorkflow {
    ...
    public static let skillMetadata: [String: String] = [
        ...
        "version": "1.0.0",
        "description": "Where new brain files go. Decision protocol for filing brain pages by primary\nsubject, not by format or source. Reference for all brain-writing skills.",
        "triggers": "\"where does this go\"\n\"filing rules\"\n\"create new page\"\n\"which directory\"",
        "tools": "search\nget_page\nlist_pages",
        "mutating": "false",
    ]
...
public struct WhenWheredoesthisgoFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenWheredoesthisgoFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("whereDoesThisGo", matching: nil))
        ...
        if __meridianShouldRun("progress:0.1:L1:C0") {
            ...
            try await runtime.emit(
                event: "trigger.whereDoesThisGo.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("where does this go"),
                ]
            )
...
public struct WhenFilingrulesFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenFilingrulesFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("filingRules", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.filingRules.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("filing rules"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenCreatenewpageFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenCreatenewpageFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("createNewPage", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.createNewPage.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("create new page"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenWhichdirectoryFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenWhichdirectoryFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("whichDirectory", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.whichDirectory.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("which directory"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
