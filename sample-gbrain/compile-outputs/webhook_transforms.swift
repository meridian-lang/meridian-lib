...
// Source: webhook_transforms.meri
...
public struct WebhookTransformsInput: MeridianWorkflow {
    ...
    public func run() async throws -> WorkflowResult {
        ...
        if __meridianShouldRun("progress:0.1:L38:C0") {
            ...
            try await runtime.checkpoint(label: "progress:0.1:L38:C0", state: state.snapshot())
        }
...
public struct WhenSetupwebhookFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenSetupwebhookFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("setUpWebhook", matching: nil))
        ...
        if __meridianShouldRun("progress:0.1:L1:C0") {
            ...
            try await runtime.emit(
                event: "trigger.setUpWebhook.fired",
                payload: [
                    "kind": .string("event"),
                    "spec": .string("set up webhook"),
                ]
            )
...
public struct WhenProcesswebhookeventFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenProcesswebhookeventFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("processWebhookEvent", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.processWebhookEvent.fired",
                payload: [
                    "kind": .string("event"),
                    "spec": .string("process webhook event"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenTransformthiseventFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenTransformthiseventFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("transformThisEvent", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.transformThisEvent.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("transform this event"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
