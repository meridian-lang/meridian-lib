...
public struct MediaIngestInput: MeridianWorkflow {
...
public struct WhenWatchthisvideoFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenWatchthisvideoFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("watchThisVideo", matching: nil))
        ...
        if __meridianShouldRun("progress:0.1:L1:C0") {
            ...
            try await runtime.emit(
                event: "trigger.watchThisVideo.fired",
                payload: [
                    "kind": .string("ambient"),
                    "spec": .string("watch this video"),
                ]
            )
...
public struct WhenProcessthisyoutubelinkFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenProcessthisyoutubelinkFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("processThisYoutubeLink", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.processThisYoutubeLink.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("process this YouTube link"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenIngestthispdfFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenIngestthispdfFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("ingestThisPdf", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.ingestThisPdf.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("ingest this PDF"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenSavethispodcastFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenSavethispodcastFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("saveThisPodcast", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.saveThisPodcast.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("save this podcast"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenProcessthisbookFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenProcessthisbookFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("processThisBook", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.processThisBook.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("process this book"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenPdfbookFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenPdfbookFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("pdfBook", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.pdfBook.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("PDF book"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenSummarizethisbookFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenSummarizethisbookFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("summarizeThisBook", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.summarizeThisBook.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("summarize this book"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenIngestitmybrainFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenIngestitmybrainFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("ingestItMyBrain", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.ingestItMyBrain.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("ingest it into my brain"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenWhatsinthisscreenshotFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenWhatsinthisscreenshotFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("whatSInThisScreenshot", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.whatSInThisScreenshot.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("what's in this screenshot"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenCheckoutthisrepoFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenCheckoutthisrepoFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("checkOutThisRepo", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.checkOutThisRepo.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("check out this repo"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
