...
public struct SkillCreatorInput: MeridianWorkflow {
    ...
    public func run() async throws -> WorkflowResult {
        ...
        if __meridianShouldRun("progress:0.2:L38:C0") {
            ...
            _ = try await runtime.invoke(
                tool: "shell.run",
                args: [
                    "command": .string("bun test test/skills-conformance.test.ts"),
                ]
            )
...
public struct WhenCreateskillFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenCreateskillFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("createSkill", matching: nil))
        ...
        if __meridianShouldRun("progress:0.1:L1:C0") {
            ...
            try await runtime.emit(
                event: "trigger.createSkill.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("create a skill"),
                ]
            )
...
public struct WhenNewskillFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenNewskillFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("newSkill", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.newSkill.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("new skill"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
...
public struct WhenImprovethisskillFires: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        await runtime.workflowStarted(workflowName: "WhenImprovethisskillFires", parameters: [:])

        if __meridianShouldRun("progress:0.0:L1:C0") {
            // L1
            try await runtime.wait(.event("improveThisSkill", matching: nil))
            try await runtime.checkpoint(label: "progress:0.0:L1:C0", state: state.snapshot())
        }
        if __meridianShouldRun("progress:0.1:L1:C0") {
            // L1
            try await runtime.emit(
                event: "trigger.improveThisSkill.fired",
                payload: [
                    "kind": .string("keyword"),
                    "spec": .string("improve this skill"),
                ]
            )
            try await runtime.checkpoint(label: "progress:0.1:L1:C0", state: state.snapshot())
        }
