...
public struct LargeReleaseTrainPullRequest: MeridianWorkflow {
    ...
    public func run() async throws -> WorkflowResult {
        ...
        if MeridianComparison.eq(state.get("ciStatus"), .string("failed")) {
        ...
        if MeridianComparison.eq(state.get("mergeStatus"), .string("conflicting")) {
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let __meridianProseResults_L38 = try await runtime.executeAutonomousLoop(
            ...
            unless: { __meridianAutonomySnapshot in
                var state = State()
                state.restore(from: __meridianAutonomySnapshot)
                return MeridianComparison.eq(state.get("mergeStatus"), .string("conflicting"))
            }
        )
