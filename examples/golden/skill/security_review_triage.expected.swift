...
// Source: security_review_triage.meridian
...
public struct SecurityReviewTriagePullRequest: MeridianWorkflow {
    public let runtime: Runtime
    public let pullRequest: PullRequest

    public static let skillMetadata: [String: String] = [
        "name": "security review triage",
        "description": "Triage a pull request from the security team's perspective with strict deterministic checks and discretion-mode planning for risky remediation.",
        "goal": "Triage security findings without taking destructive automated actions.",
    ...
    public func run() async throws -> WorkflowResult {
        ...
        await runtime.workflowStarted(workflowName: "SecurityReviewTriagePullRequest", parameters: [:])
        ...
        for (__meridianLoopIndex_0_4, comment) in (state.get("comments")?.asList ?? []).enumerated() {
            ...
            state.bind("comment", comment)
                if __meridianShouldRun("progress:0.4.body.0:L20:C0") {
                    _ = try await TriageComment(runtime: runtime, comment: (try Value.from(comment).coerce(to: Value.self))).run()
...
public struct TriageComment: MeridianWorkflow {
    public let runtime: Runtime
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        let instances = Instances()
        await runtime.workflowStarted(workflowName: "TriageComment", parameters: [:])

        if try await runtime.discretion.decide(DiscretionContext(question: "the comment still applies", snapshot: state.snapshot())) {
            if __meridianShouldRun("progress:0.0.then.0:L24:C0") {
                ...
                try await runtime.checkpoint(label: "progress:0.0.then.0:L24:C0", state: state.snapshot())
            }
        } else {
            if __meridianShouldRun("progress:0.0.else.0:L174:C0") {
                try await runtime.emit(
                    ...
                    payload: [
                        "pr": state.get("pullRequest.number") ?? .null,
                        "reason": .string("comment auto-resolved by triage"),
                    ]
                )
...
public struct PlanSecurityRemediation: MeridianWorkflow {
    public let runtime: Runtime
    public let pullRequest: PullRequest
    ...
    public func run() async throws -> WorkflowResult {
        ...
        let constants = Constants()
        let instances = Instances()
        await runtime.workflowStarted(workflowName: "PlanSecurityRemediation", parameters: [:])

        let __meridianProseResults_L31 = try await runtime.executeProsePlan(
            prose: "Inspect the merge status, vulnerability scan results, and risk score",
        ...
        let __meridianProseResults_L32 = try await runtime.executeProsePlan(
            prose: "Propose only auditable remediation steps using request reviewer approval, open remediation task, and notify channel",
            snapshot: state.snapshot(),
            scopedTools: ["applySmallestRepair", "draftSupportReply", "escalateCustomerTicket", "evaluatePolicy", "getCIStatus", "getDeploymentHealth", "getFailingJobs", "getMergeStatus", "getPullRequest", "haltRelease", "listOutdatedDependencies", "listPullRequestComments", "mitigateService", "notifyChannel", "openIncident", "openRemediationTask", "pageOncall", "promoteDeployment", "pushBranch", "readCustomerProfile", "readCustomerTicket", "recordAuditNote", "requestReviewerApproval", "resolveComment", "rollbackDeployment", "runTestSuite", "scanVulnerabilities", "sendSupportReply", "snapshotServiceHealth", "syncBranch", "updateIncident", "updateRemediationTask", "upgradeDependency"]
        )
        ...
        let __meridianProseResults_L33 = try await runtime.executeProsePlan(
            prose: "Never propose automatic merging, branch deletion, or rollbacks",
            snapshot: state.snapshot(),
            scopedTools: ["applySmallestRepair", "draftSupportReply", "escalateCustomerTicket", "evaluatePolicy", "getCIStatus", "getDeploymentHealth", "getFailingJobs", "getMergeStatus", "getPullRequest", "haltRelease", "listOutdatedDependencies", "listPullRequestComments", "mitigateService", "notifyChannel", "openIncident", "openRemediationTask", "pageOncall", "promoteDeployment", "pushBranch", "readCustomerProfile", "readCustomerTicket", "recordAuditNote", "requestReviewerApproval", "resolveComment", "rollbackDeployment", "runTestSuite", "scanVulnerabilities", "sendSupportReply", "snapshotServiceHealth", "syncBranch", "updateIncident", "updateRemediationTask", "upgradeDependency"]
        )
